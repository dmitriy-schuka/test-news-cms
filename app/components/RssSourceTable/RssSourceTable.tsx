...
const RssSourceTable = ({ rssData, page, sortDirection, sortColumn, hasNextPage, hasPreviousPage, handleEditRssSource }) => {
  ...
  const rowMarkup = useMemo(() => {
    if (checkIsArray(sortedRss)) {
      return (
        sortedRss.map((rss, index) => {
          const { id, name, url, isActive, fieldMapping, stopTags, importInterval } = rss;

          return (
            <IndexTable.Row key={`${id}_rss`} id={id} position={index} onClick={() => handleEditRssSource(id)}>
              <IndexTable.Cell>
    ...
    <Box>
      <Card>
        <IndexTable
          ...
          emptyState={<EmptyStateMarkup resourceName={resourceName.plural}/>}
          onSort={handleSort}
        >
          {rowMarkup}
        </IndexTable>
