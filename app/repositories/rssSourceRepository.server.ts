...
import type { RssSource, RssSourceCreate, RssSourceUpdate } from "~/@types/rssSource";
import { deleteRssNewsBySourceId, storeImportedRssNews } from "~/repositories/rssNewsRepository.server";
...
export const mapFields = (item: any, fieldMapping: Record<string, string>) => {
...
export const fetchAndStoreRssFeeds = async () => {
  ...
  if (activeSources?.length) {
    for (const source of activeSources) {
      try {
        /** Check interval */
        const difference = source.fetchedAt ? differenceInMinutes(new Date, new Date(source.fetchedAt)) : 0;

        if (source.importInterval && difference >= Number(source.importInterval)) {
          ...
          const latestTwoRssNews = jsonData?.items?.splice(0, 2) || [];

          for (const item of latestTwoRssNews) {
            if (source.stopTags.some(tag => item.categories?.includes(tag))) {
              continue;
            }
