...
export const loader: LoaderFunction = async ({ params }) => {
    const rssSource =
        params?.id !== 'new' ? await getRssSourceById(Number(params.id)) : null;
...
export const action: ActionFunction = async ({ request }) => {
    ...
    try {
        switch (request.method) {
            ...
            case 'PUT': {
                await updateRssSource(Number(id), {
                    name,
                    url,
                    isActive,
                    fieldMapping,
...
export default function RssSourceForm() {
    ...
    const [fieldMapping, setFieldMapping] = useState(
        JSON.stringify(rssData?.fieldMapping || {})
    );
    const [stopTags, setStopTags] = useState(
        JSON.stringify(rssData?.stopTags || [])
    );
        ...
        >
            <Form method={rssData?.id ? 'put' : 'post'}>
                <Card sectioned>
                    ...
                    {rssData?.id && (
                        <input type="hidden" name="id" value={rssData.id} />
                    )}
                    <TextField
                        label="Name"
                        name="name"
                    ...
                    <TextField
                        ...
                        onChange={(value) =>
                            handleChange(value, 'importInterval')
                        }
                        autoComplete="off"
                    />
                    <Checkbox
