...
import {
    ...
    getUserById,
    updateUser,
} from '~/repositories/userRepository.server';
        ...
        switch (request.method) {
            case 'PUT': {
                ...
                if (user?.id) {
                    ...
                    return json(
                        { error: 'Error update account data' },
                        { status: 400 }
                    );
                }
            }
...
export default function Account() {
    ...
    }, [actionData, updateState]);

    useEffect(() => {
        if (loaderData?.user) {
            updateState('userData', loaderData.user);
        }
    }, [loaderData, updateState]);

    const handleChange = (fieldName: keyof User, value: string | number) => {
        setState((prev) => ({
            ...prev,
    ...
    return (
        <Page title={'Account page'} titleHidden>
            <AccountForm
