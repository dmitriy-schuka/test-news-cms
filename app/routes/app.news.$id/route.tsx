...
import {
    ...
    updateNews,
    restoreNews,
} from '~/repositories/newsRepository.server';
        ...
        switch (request.method) {
            case 'POST': {
                ...
                if (news?.id) {
                    ...
                    return json(
                        { error: 'Error creating news data' },
                        { status: 400 }
                    );
                }
            }
            ...
            case 'PATCH': {
                const news = await restoreNews(Number(newsId));

                if (news?.id) {
                    return redirect('/app/news/list');
                } else {
                    return json(
                        { error: 'Error restoring news' },
                        { status: 400 }
                    );
                }
            }
...
export default function News() {
    ...
    const prepareFormData = useCallback((newsData: News) => {
    ...
    const handleDeleteNews = useCallback(() => {
        ...
        newsData.media &&
            formData.append('media', JSON.stringify(newsData.media));

        submit(formData, { method: 'DELETE', encType: 'multipart/form-data' });
    }, [newsData, submit]);

    const handleRestoreNews = useCallback(() => {
        const formData = new FormData();
